...
class ITjuziSpider(RedisCrawlSpider):
    ...
    def parse_item(self, response):
        global company_name, slogan, scope, sub_scope, city, area, home_page, tags, company_intro, company_full_name, found_time, company_size, company_status
        print("======================" + response.url)
        # 做网页解析
        ...
        if pdt:
            for li in pdt.find_all('li'):
                pdt_dict = {'pdt_name': li.find('h4').b.get_text().strip(),
                            'pdt_type': li.find('span', class_='tag yellow').get_text().strip(),
                            'pdt_intro': li.find(class_='on-edit-hide').p.get_text().strip()}
